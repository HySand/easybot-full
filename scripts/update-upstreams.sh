...
[[ "$qq_deb_url" =~ ^https://(qqdl\.gtimg\.cn|dldir1(v6)?\.qq\.com)/qqfile/[A-Za-z0-9._/-]+/linuxqq_[0-9]+\.[0-9]+\.[0-9]+-[0-9]+_[0-9]+_(amd64|arm64)\.deb$ ]] \
  || die 'invalid official QQ deb URL'
[[ "$qq_version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || die 'invalid QQ version'
[[ "$qq_deb_url" == *"linuxqq_${qq_version}-"* ]] || die 'QQ URL and version do not match'
...
qq_url_pattern='^ARG QQ_DEB_URL=https://(qqdl\.gtimg\.cn|dldir1(v6)?\.qq\.com)/qqfile/[A-Za-z0-9._/-]+/linuxqq_[0-9]+\.[0-9]+\.[0-9]+-[0-9]+_[0-9]+_(amd64|arm64)\.deb$'
