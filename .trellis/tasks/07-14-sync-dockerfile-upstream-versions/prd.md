...
- R2：Dockerfile 锁定 .NET 基础镜像标签、EasyBot commit、Chrome 版本、NapCat-Docker commit、NapCat release 和 QQ 版本；Linux QQ deb URL 在构建时通过腾讯官方 latest 配置解析，避免已下架直链导致重建失败。
...
- R6：GitHub Actions 每 6 小时及手动触发，查询两个仓库的 main commit、NapCatQQ latest stable release 与腾讯 Linux QQ 官方 latest 配置，并解析 .NET、Chrome、QQ 版本与当前下载 URL 摘要。
